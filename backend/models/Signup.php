...
class Signup extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['member_id', 'academy_id', 'avatar', 'real_name', 'card', 'phone', 'email', 'province', 'city', 'area', 'sex', 'birthday', 'marital', 'emergency_name', 'emergency_phone'], 'required'],
            [['member_id', 'academy_id', 'status', 'created_at', 'updated_at'], 'integer'],
            [['education_start_time', 'education_end_time'], 'safe'],
            [['answer_1', 'answer_2', 'answer_3', 'status_reason'], 'string'],
            [['avatar', 'card_front', 'card_back', 'diploma_scan', 'academic_scan', 'postgraduate_diploma_scan', 'postgraduate_academic_scan', 'report_card', 'postgraduate_report_card', 'resume_chinese', 'company_organization_chart', 'resume_english', 'english_essays', 'additional_material', 'interview_batch'], 'string', 'max' => 100],
            ...
            [['card'], 'string', 'max' => 18],
            [['email', 'university'], 'string', 'max' => 50],
    ...
    public function exec_number(){
        $result = Signup::findBySql('SELECT count(1) as num_count,status FROM yii_signup where status >= 123456 group by status')->asArray()->all();
        $ret = array(
            'uncheck' => '0',
            'passed' => '0',
            'forbid' => '0'

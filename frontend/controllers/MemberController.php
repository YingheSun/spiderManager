...
class MemberController extends BaseController
{
    ...
    public function actionEmail(){
        if(Yii::$app->session->has('member')) {
            ...
            $member = \frontend\models\Member::getInfo($member);
            if($member->email) {
                $email = self::gotomail($member->email);
                return $this->redirect('http://'.$email);
            }
            $this->redirect(Url::toRoute(['member/index'],self::$domain));
        }else{
            $this->redirect(Url::toRoute(['site/index'],self::$domain));
        }
    }
    ...
    private static function gotomail($mail){
        $t=explode('@',$mail);
        $t=strtolower($t[1]);
        if($t=='163.com'){
            return 'mail.163.com';
